...
// Skapar config, registry, footsteps och param-lista
...
namespace AudioKit.FMOD
{
    public static class AudioTools
    {
        ...
        [MenuItem("AudioKit/Ljud/Skapa AudioParamLibrary i Resources")]
        public static void CreateParamLibrary()
        {
            EnsureFolder(ResourcesAudio);

            var path = Path.Combine(ResourcesAudio, "AudioParamLibrary.asset").Replace("\\", "/");
            var existing = AssetDatabase.LoadAssetAtPath<AudioParamLibrarySO>(path);
            if (existing != null)
            {
                Selection.activeObject = existing;
                return;
            ...
            var asset = ScriptableObject.CreateInstance<AudioParamLibrarySO>();
            AssetDatabase.CreateAsset(asset, path);
            AssetDatabase.SaveAssets();
            Selection.activeObject = asset;
        }
